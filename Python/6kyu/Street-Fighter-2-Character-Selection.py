...
def street_fighter_selection(fighters, initial_position, moves):
    selected = []
    currPos = [initial_position[0],initial_position[1]]
    for i in moves:
        if i == "up":
            if currPos[0] == 1: currPos[0] -= 1
        if i == "down":
            if currPos[0] == 0: currPos[0] += 1
        if i == "right":
            if currPos[1] == len(fighters[0]) - 1: currPos[1] = 0
            else: currPos[1] += 1
        if i == "left":
            if currPos[1] == 0: currPos[1] = len(fighters[0]) - 1
            else: currPos[1] -= 1
        selected.append(fighters[currPos[0]][currPos[1]])
    return selected