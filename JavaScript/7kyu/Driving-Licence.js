// https://www.codewars.com/kata/586a1af1c66d18ad81000134/javascript
...
function driver(data) {
	...
	let dlpart1 = [9, 9, 9, 9, 9];
	for (let i = 0; i < 5 && i < data[2].length; i++) {
		dlpart1[i] = data[2][i].toUpperCase();
	}

	let dlpart2 = +data[3].split("-")[2].split("")[2];

	let dlpart3 =
		data[4] === "F"
			? +monthMap[data[3].split("-")[1].substring(0, 3).toLowerCase()] +
			  50
			: monthMap[data[3].split("-")[1].substring(0, 3).toLowerCase()];

	let dlpart4 = data[3].split("-")[0];

	let dlpart5 = +data[3].split("-")[2].split("")[3];

	let dlpart6 = data[0].split("")[0];
	let dlpart7 = data[1].length > 0 ? data[1].split("")[0] : 9;
