...
function findSenior(list)
{
    ...
    {
        if(maxAge < list[i].age){maxAge = list[i].age;}
    }
    for(var i = 0; i < list.length; i++)
    {
        if(list[i].age == maxAge)
        {      
            arr.push({'firstName': list[i].firstName, 'lastName': list[i].lastName, 'country': list[i].country,
                    'continent': list[i].continent, 'age': list[i].age, 'language': list[i].language});
        }
    }
    return arr;
}