// https://www.codewars.com/kata/5508249a98b3234f420000fb/javascript
...
const getCharCodeRange = (c) =>
	/[A-Z]/.test(c) ? [65, 90] : /[a-z]/.test(c) ? [97, 122] : [0, 0];
const shiftChar = (c, s, dir) => {
	const [a, z] = getCharCodeRange(c);
	if (!a) return c;
	s %= 26;
	let code = c.charCodeAt() + dir * s;
	while (code < a) code = z - (a - code) + 1;
	while (code > z) code = a + (code - z) - 1;
	return String.fromCharCode(code);
};
const code = (c, s) => shiftChar(c, s, 1);
const decode = (c, s) => shiftChar(c, s, -1);
const movingShift = (str, shift) =>
	str2arr([...str].reduce((a, c, i) => a + code(c, i + shift), ""));
const demovingShift = (arr, shift) =>
	[...arr.join("")].reduce((a, c, i) => a + decode(c, i + shift), "");
const str2arr = (str) => {
	const len = Math.ceil(str.length / 5);
	return Array.from({ length: 5 }, (_, i) => str.substr(i * len, len));
};
